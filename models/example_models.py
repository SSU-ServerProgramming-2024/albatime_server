...
class WorkInformation(db.Model):
    ...
    bossno = db.Column(db.BigInteger, db.ForeignKey('boss.bossno'), nullable=False)  # boss 테이블과의 관계를 가정
    reviewno = db.Column(db.BigInteger, db.ForeignKey('employee_review.reviewno'), nullable=True)
    startday = db.Column(db.Date, nullable=False)
    money = db.Column(db.Integer, nullable=False)
    comno = db.Column(db.BigInteger, db.ForeignKey('company.comno'), nullable=False)  # company 테이블과의 관계를 가정
